...
Предполагается, что длительности обслуживания различных требований могут быть зависимыми и иметь различные законы распределения, поэтому вместо классического способа, состоящего в указании функции распределения длительности обслуживания произвольного требования, будут использованы потоки насыщения. Потоки насыщения $\Pi^{\mathrm{\text{нас}}}_j$, $j \in \{1,2,3,4\}$, определяются как виртуальные выходные потоки при условии максимального использования ресурсов обслуживающего устройства, а для $j\in \{1, 2, 3\}$ еще и при условии максимальной загрузки соответствующих очередей. Поток насыщения $\Pi^{\mathrm{\text{нас}}}_j$, $j\in \{1,2,3\}$, будет содержать неслучайное число $\ell_{k,r,j}$ требований, обслуженных в течение времени $T^{(k,r)}$, если обслуживающее устройство находится в состоянии $\Gamma^{(k,r)}$. Пусть $\mathbb{Z}_+$~--- множество целых неотрицательных чисел. Тогда, при условии, что в очереди $O_4$ находится $x \in \mathbb{Z}_+$ требований, поток насыщения $\Pi^{\mathrm{\text{нас}}}_4$ определим как поток, содержащий все $x$ требований.
...
%\ml
%{
%\Pr \left(\{ \omega \colon \eta_{3,i} = a_3, \xi_{3,i}=b_3\} \left|\bigcap_{t=0}^{i}\{\omega\colon \Gamma_t=\Gamma^{(k_t,r_t)}, \varkappa_t=x^t\}\right.\right)=\\
%=\Pr \left(\{ \omega \colon \eta_{3,i} = a_3, \xi_{3,i}=b_3\} \left|\bigcap_{t=0}^{i}\{\omega\colon \Gamma_t=\Gamma^{(k_t,r_t)}, \varkappa_{3,t}=x_3^t\}\right.\right)=\\
%=\Pr \left(\{ \omega \colon \eta_{3,i} = a_3, \xi_{3,i}=b_3\} \left|\{\omega\colon \Gamma_i=\Gamma^{(k_i,r_i)}, \varkappa_{3,i}=x_3^i\}\right.\right)
...
%Аналогично, из \eqref{ProbablititiesToProve} можно заключить, что вероятность $\Pr (\{ \omega \colon \eta_{3,i} = a_3, \xi_{3,i}=b_3\} |\bigcap_{t=0}^{i}\{\omega\colon \Gamma_t=\Gamma^{(k_t,r_t)}, \varkappa_t=x^t\})$ равна величине $\varphi_3(a_3,h_T(\ga{{k_i},{r_i}},x_3^i))\times \delta_{b_3,\ell(\tilde{k},\tilde{r},3)}$, $\Gamma^{(\tilde{k},\tilde{r})}=h(\Gamma^{(k_i,r_i)},x_3^i)$, зависящей только от значений $(\Gamma^{(k_i,r_i)},x_3^i)$ пары $(\Gamma_i,\varkappa_{3,i})$. Следовательно, равенства \eqref{eta:xi:three:forgetProperty} также верны.
\end{proof}
...
\begin{corollary}
Пусть $\Gamma^{(k_{i+1},r_{i+1})}=h(\Gamma^{(k_i,r_i)},x_{3,i})$. Тогда в условиях теоремы~\ref{myTheorem} верны равенства
\begin{align}
...
= \sum_{a_2,b_2\in \mathbb{Z}_+}\psi(a_2,x_{4,i}, p_{k_{i+1},r_{i+1}})  \delta_{b_2,\ell(k_{i+1},r_{i+1},2)}   \delta_{x_{2,i+1},\max\{0,x_{2,i}+a_2-b_2\}} \times \sum_{a_3\in \mathbb{Z}_+} \varphi_3(a_3,h_T(\Gamma^{(k_i,r_i)},x_{3,i})) \times \\
\times \sum_{a_1\in \mathbb{Z}_+} \varphi_1(a_1,h_T(\Gamma^{(k_i,r_i)},x_{3,i})) \sum_{a_4\in \mathbb{Z}_+} \delta_{a_4,\min{\{\ell(k_{i+1},r_{i+1},1), x_{1,i}+a_1}\}} \sum_{b_1\in \mathbb{Z}_+} \delta_{b_1,\ell(k_{i+1},r_{i+1},1)} \sum_{b_3\in \mathbb{Z}_+} \delta_{b_3,\ell(k_{i+1},r_{i+1},3)} \times \\
...
\end{theorem}
\begin{proof}
%Для нахождения переходной вероятности $\Pr(\Gamma_{i+1}=\Gamma^{(\tilde{k},\tilde{r})},\varkappa_{i+1}=\tilde{x}|\Gamma_{i}=\Gamma^{(k,r)},\varkappa_i=x) $ воспользуемся формулой полной вероятности. Выделим из множества событий вида $\{\eta_i=a;\xi_i=b\}$, $a$, $b\in \mathbb{Z}_+^4$, полную группу несовместных событий, при условии наступления события $\{\Gamma_{i}=\Gamma^{(k,r)},\varkappa_i=x\}$, вероятности которых будут положительны. По определению, функции $\varphi_j(x,t)$, $j\in \{1,3\}$, положительны тогда и только тогда, когда $x \in \mathbb{Z}_+^4$. Аналогично, значение функции $\psi(s,m,u)$ положительно только в случае $0\leqslant s \leqslant m$. Тогда из формул \eqref{ProbablititiesToProve}, \eqref{conditionProbOne} и \eqref{conditionProbTwo} следует, что вероятность $\Pr(\eta_i=a, \xi_i=b|\Gamma_{i}=\Gamma^{(k,r)},\varkappa_i=x)$ положительна тогда и только тогда, когда 
%\begin{equation}
...
%Теперь найдем такие $a_1$, $a_2$ и $a_3$, при которых положительны вероятности $\Pr (\Gamma_{i+1}=\Gamma^{(\tilde{k},\tilde{r})},\varkappa_{i+1}=\tilde{x}|\Gamma_{i}=\Gamma^{(k,r)},\varkappa_i=x, \eta_i=a; \xi_i=b)$. Из \eqref{queuesFunc} и \eqref{FourthFunc} получаем следующие условия на $a_1$, $a_2$, $a_3$ и дополонительное условие на $a_4$:
%\begin{equation}
%\begin{aligned}
...
%\ml
%{
%\P{\Gamma_{i+1}=\Gamma^{(\tilde{k},\tilde{r})},\varkappa_{i+1}=\tilde{x}}{\Gamma_{i}=\Gamma^{(k,r)},\varkappa_i=x} = \widetilde{\varphi}_1(\tilde{k},\tilde{r},h_T(\Gamma^{(k,r)},x_3),x_1,\tilde{x}_1) \times  \\ 
%\times
%\widetilde{\psi}(\tilde{k},\tilde{r},x_4,x_2, \tilde{x}_2) 
%\times
%\widetilde{\varphi}_2(\tilde{k},\tilde{r},h_T(\Gamma^{(k,r)}),x_3,\tilde{x}_3)
%\times \delta_{a_4,\min{\{\ell(\tilde{k},\tilde{r},1), x_1+a_1}\}},
%}
...
вероятность за конечное число переходов из входного состояния цикла $(\Gamma^{(k,0)}, (0,0,x_{3,1},0))$ попасть в произвольное состояние цикла $(\Gamma^{(k,r)},x_2)$ положительна.
\label{lemma:3}
\end{lemma}
\begin{proof}
Поскольку с ненулевой вероятностью по потоку $\Pi_3$ на последующих тактах может приходить любое количество требований, положим, что на всех тактах, когда обслуживающее устройство находится в выходном состоянии $\Gamma^{(k,n_k)}$, в очередь $O_3$ приходит ровно $\sum_{t=0}^{n_k} \ell_{k,t,3}$ требований. Причем на остальных тактах по потоку $\Pi_3$ требования не поступают. Этим обеспечивается сколь угодно долгое пребывание прибора в цикле $C_k$, при этом сохраняя количество требований в очереди $O_3$ неизменным и равным $x_{3,1}$.

Далее допустим, что на первом же такте (то есть в состоянии $(\Gamma^{(k,0)}, (0,0,x_{3,1},0))$) по потоку $\Pi_1$ пришло $x_{2,2}+x_{4,2} + \ell_{k,r\ominus_k 1,2} - \ell_{k,r\ominus_k 1,1}$ требований, которые все спустя несколько тактов перешли в очередь $O_4$. При этом перемещения требований из очереди $O_4$ в очередь $O_2$ не происходило. В состоянии $(0, 0, x_{3,2}, x_{2,2}+x_{4,2} + \ell_{k,r\ominus_k 1,2} - \ell_{k,r\ominus_k 1,1})$ очереди могут оставаться сколь угодно долгоо, вплоть до такта, в котором обслуживающее устройство  будет находиться в состоянии $\Gamma^{(k,r\ominus_k 1)}$. На этом такте по потоку $\Pi_1$ придет $x_{1,2}+\ell_{k,r\ominus_k 1,1}$ требований и из очереди $O_4$ уйдет $x_{2,2}+\ell_{k,r\ominus_k 1,2}$ требований. Таким образом, на следующем такте состояние системы будет $(\Gamma^{(k,r)},(x_{1,2},x_{2,2},x_{3,1}-\sum_{t=0}^{r-1}\ell_{k,t,3},x_{4,2}))$.
\end{proof}

Из лемм \ref{lemma:2} и \ref{lemma:3} вытекает следующий результат.
\begin{lemma}
Для любых $x_{3,1} \leqslant L$, $x_2 \in X^{(k,r)} \cap \{x_2\in \mathbb{Z}_+^4 \colon x_{3,2} > L - \sum_{t=0}^{r-1}\ell_{k,t,3}\}$, $0 \leqslant r_1 \leqslant n_0$, $0 \leqslant r_2 \leqslant n_k$, $1 \leqslant k \leqslant d$ вероятность попасть за конечное число переходов из состояния продления $(\Gamma^{(0,r_1)},(0,0,x_{3,1},0))$ в состояние цикла $(\Gamma^{(k,r_2)},x_2)$ положительна.
\label{lemma:4}
\end{lemma}

\begin{lemma}
Для любых $\Gamma^{(k_1,r_1)}, \Gamma^{(k_2,r_2)} \in \Gamma$ и $x_1 \in \mathbb{Z}_+^4, x_2 \in \mathbb{Z}_+^4 \setminus X^{(k_1,r_1)}$ вероятность перейти на следующем такте из состояния $(\Gamma^{(k_1,r_1)},x_1)$ в состояние $(\Gamma^{(k_2,r_2)}, x_2)$ равна нулю.
\label{lemma:5}
\end{lemma}
\begin{proof}
...
\begin{equation*}
H_{-1}(\Gamma^{(k,r)}, x_3) = \{\gamma \in \Gamma \colon h(\gamma, x_3) = \Gamma^{(k,r)}\}.
\end{equation*}
...
\begin{theorem}
Пусть $\tilde{\gamma} =\Gamma^{(\tilde{k},\tilde{r})}\in \Gamma$ и $\tilde{x}_3 \in Z_+$. Тогда для переходных вероятностей $\{Q_{3,i}(\cdot,\cdot)\}_{i\geqslant 0}$ марковской цепи $\MarkThree$ имеют место следующие рекуррентные соотношения:
\begin{multline}
Q_{3,i+1}(\tilde{\gamma},\tilde{x}_3) = (1-\delta_{\tilde{x}_3,0}) \sum_{x_3=0}^{\tilde{x}_3 +  \ell(\tilde{k},\tilde{r},3)}\sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) \times 
...
\begin{multline*}
Q_{3,i+1}(\tilde{\gamma},\tilde{x}_3)= \sum_{x_3=0}^{\infty}\sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) \times 
...
Поскольку  $\varphi_3(x,t)=0$, для $x<0$, в результате получаем
\begin{multline*}
Q_{3,i+1}(\tilde{\gamma},\tilde{x}_3)= (1-\delta_{\tilde{x}_3,0}) \sum_{x_3=0}^{\tilde{x}_3 +  \ell(\tilde{k},\tilde{r},3)}\sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) \times 
\varphi_3(\tilde{x}_3 + \ell(\tilde{k},\tilde{r},3) - x_3,T^{(\tilde{k},\tilde{r})}) + \\
+ \delta_{\tilde{x}_3,0} \sum_{x_3=0}^{\ell(\tilde{k},\tilde{r},3)}\sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) \sum_{a=0}^{\ell(\tilde{k},\tilde{r},3) - x_3} \varphi_3(a,T^{(\tilde{k},\tilde{r})}).
...
\begin{multline}
\alpha_i(0,\tilde{r},v) =\tilde{\alpha}_i(0,\tilde{r},v) + q_{0,\tilde{r}}(v) \times \sum_{x_3=0}^{L} \left[ Q_{3,i}(\Gamma^{(k_1,r_1)},x_3) + Q_{3,i}(\Gamma^{(0,r\ominus_0 1)},x_3) \right] v^{x_3}  - \\ - q_{\tilde{k},\tilde{r}}(v)\sum_{x_3=0}^{L} Q_{3,i}(\Gamma^{(0,r\ominus_0 1)},x_3), \quad \Gamma^{(0,\tilde{r})} \in \Gamma.
\end{multline}

\begin{equation}
\alpha_i(\tilde{k},\tilde{r},v) =\tilde{\alpha}_i(\tilde{k},\tilde{r},v) - q_{\tilde{k},\tilde{r}}(v)\sum_{x_3=0}^{L} \left[ Q_{3,i}(\Gamma^{(k,r\ominus_k 1)},x_3) + Q_{3,i}(\Gamma^{(0,r_2)},x_3) \right] v^{x_3}, \quad \Gamma^{(\tilde{k}, \tilde{r})} \in C_{\tilde{k}}^{\mathrm{I}}
\end{equation}
...
\item для $ \Gamma^{(0,\tilde{r})} \in \Gamma$ $$\mathfrak{M}^{(i+1)}(0,\tilde{r},v) = q_{0,\tilde{r}} (v)\times \mathfrak{M}^{(i)}(0,\tilde{r} \ominus_0 1,v) + \alpha_i(0,\tilde{r},v);$$
\item для $\Gamma^{(\tilde{k}, \tilde{r})} \in C_{\tilde{k}}^{\mathrm{I}}$ и $h_3(r_2)=\Gamma^{(\tilde{k},\tilde{r})}$
$$\mathfrak{M}^{(i+1)}(\tilde{k},\tilde{r},v) = q_{\tilde{k},\tilde{r}} (v)\times \left[ \mathfrak{M}^{(i)}(\tilde{k},\tilde{r} \ominus_{\tilde{k}} 1,v) +  \mathfrak{M}^{(i)}(0,r_2,v) \right] + \alpha_i(\tilde{k},\tilde{r},v);$$
...
\begin{multline}
\mathfrak{M}^{(i+1)}(\tilde{k},\tilde{r},v) = \sum_{x_3=0}^{\ell(\tilde{k},\tilde{r},3)}\sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) \sum_{a=0}^{\ell(\tilde{k},\tilde{r},3) - x_3} \varphi_3(a,T^{(\tilde{k},\tilde{r})}) + \\
+ \sum_{w=1}^{\infty} \sum_{x_3=0}^{w +  \ell(\tilde{k},\tilde{r},3)}\sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) \times 
...
\begin{equation*}
\sum_{w=1}^{\infty} \sum_{x_3=0}^{w+\ell(\tilde{k},\tilde{r},3)} (\dots) = \sum_{x_3=0}^{\ell(\tilde{k},\tilde{r},3)}\sum_{w=1}^{\infty}(\dots) + \sum_{x_3=\ell(\tilde{k},\tilde{r},3) + 1}^{\infty}\sum_{w=x_3-\ell(\tilde{k},\tilde{r},3)}^{\infty}(\dots).
\end{equation*}
Преобразуем их.
\begin{multline}
 \sum_{x_3=0}^{\ell(\tilde{k},\tilde{r},3)}\sum_{w=1}^{\infty}  \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) \times 
\varphi_3(w + \ell(\tilde{k},\tilde{r},3) - x_3,T^{(\tilde{k},\tilde{r})}) v^w = \\
=  \sum_{x_3=0}^{\ell(\tilde{k},\tilde{r},3)}  \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3)  \sum_{w=1}^{\infty} 
\varphi_3(w + \ell(\tilde{k},\tilde{r},3) - x_3,T^{(\tilde{k},\tilde{r})}) v^w = \\
=\sum_{x_3=0}^{\ell(\tilde{k},\tilde{r},3)}  \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) v^{x_3-\ell(\tilde{k},\tilde{r},3)}  \sum_{w=\ell(\tilde{k},\tilde{r},3) + 1 -x_3}^{\infty}
\varphi_3(w,T^{(\tilde{k},\tilde{r})}) v^w.
\label{sum:one}
\end{multline}
Аналогично для второго слагаемого
\begin{multline}
 \sum_{x_3=\ell(\tilde{k},\tilde{r},3) + 1}^{\infty}\sum_{w=x_3-\ell(\tilde{k},\tilde{r},3)}^{\infty} \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) \times 
\varphi_3(w + \ell(\tilde{k},\tilde{r},3) - x_3,T^{(\tilde{k},\tilde{r})}) v^w = \\
=  \sum_{x_3=\ell(\tilde{k},\tilde{r},3) + 1}^{\infty} \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) v^{x_3-\ell(\tilde{k},\tilde{r},3)}\sum_{w=0}^{\infty}  
\varphi_3(w,T^{(\tilde{k},\tilde{r})}) v^w = \\
= \sum_{x_3=0}^{\infty} \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) v^{x_3-\ell(\tilde{k},\tilde{r},3)}\sum_{w=0}^{\infty} 
\varphi_3(w,T^{(\tilde{k},\tilde{r})}) v^w - \\
- \sum_{x_3=0}^{\ell(\tilde{k},\tilde{r},3)} \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) v^{x_3-\ell(\tilde{k},\tilde{r},3)}\sum_{w=0}^{\infty}
\varphi_3(w,T^{(\tilde{k},\tilde{r})}) v^w
\label{sum:two}
\end{multline}

Подставляя \eqref{sum:one} и \eqref{sum:two} в \eqref{sum:zero}, получим:
\begin{multline*}
\mathfrak{M}^{(i+1)}(\tilde{k},\tilde{r},v) = \sum_{x_3=0}^{\ell(\tilde{k},\tilde{r},3)}\sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) \sum_{a=0}^{\ell(\tilde{k},\tilde{r},3) - x_3} \varphi_3(a,T^{(\tilde{k},\tilde{r})}) + \\
+ \sum_{x_3=0}^{\ell(\tilde{k},\tilde{r},3)}  \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) v^{x_3-\ell(\tilde{k},\tilde{r},3)}  \sum_{w=\ell(\tilde{k},\tilde{r},3) + 1 -x_3}^{\infty}
\varphi_3(w,T^{(\tilde{k},\tilde{r})}) v^w + \\
...
\varphi_3(w,T^{(\tilde{k},\tilde{r})}) v^w ] +\\
+ q_{\tilde{k},\tilde{r}}(v) \sum_{x_3=0}^{\infty} \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) v^{x_3} = \tilde{\alpha}_i(\tilde{k},\tilde{r},v) + q_{\tilde{k},\tilde{r}}(v) \sum_{x_3=0}^{\infty} \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) v^{x_3},
\end{multline*}
где всю сумму, без учета последнего слагаемого, мы обозначили через $\tilde{\alpha}_i(\tilde{k},\tilde{r},v) $.

Рассмотрим более подробно сумму $\sum_{x_3=0}^{\infty} \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) v^{x_3}$ в зависимости от $\tilde{\gamma}$.

В случае, если $\tilde{\gamma} = \Gamma^{(0,\tilde{r})}$, то из определения следует, что $H_{-1}(\tilde{\gamma},x_3) = \{\Gamma^{(k_1,r_1)}, \Gamma^{(0,r\ominus_0 1)}\}$ для $x_3 \leqslant L$ и $H_{-1}(\tilde{\gamma},x_3) = \{\Gamma^{(0,r\ominus_0 1)}\}$ для $x_3 > L$. Здесь пара $(k_1,r_1)$ такая, что $h_1(\Gamma^{(k_1,r_1)}) = \tilde{r}$. Тогда рассматриваемая сумма примет вид
\begin{multline}
\sum_{x_3=0}^{\infty} \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) v^{x_3} = \sum_{x_3=0}^{L} \left[ Q_{3,i}(\Gamma^{(k_1,r_1)},x_3) + Q_{3,i}(\Gamma^{(0,r\ominus_0 1)},x_3) \right] v^{x_3}  + \\ + \sum_{x_3=L+1}^{\infty} Q_{3,i}(\Gamma^{(0,r\ominus_0 1)},x_3) v^{x_3} = \sum_{x_3=0}^{L} \left[ Q_{3,i}(\Gamma^{(k_1,r_1)},x_3) + Q_{3,i}(\Gamma^{(0,r\ominus_0 1)},x_3) \right] v^{x_3}  -\\- \sum_{x_3=0}^{L} Q_{3,i}(\Gamma^{(0,r\ominus_0 1)},x_3)  + \sum_{x_3=0}^{\infty} Q_{3,i}(\Gamma^{(0,r\ominus_0 1)},x_3) v^{x_3}.
\end{multline}
В случае, если $\tilde{\gamma} \in C_{\tilde{k}}^{\mathrm{I}}$, то из определения находим, что $H_{-1}(\tilde{\gamma},x_3) = \emptyset$ для $x_3 \leqslant L$ и $H_{-1}(\tilde{\gamma},x_3) = \{\Gamma^{(\tilde{k},\tilde{r}\ominus_{\tilde{k}} 1)}, \Gamma^{(0,r_2)}\}$ для $x_3 > L$. Здесь $r_2$ таково, что $h_3(r_2)=\Gamma^{(\tilde{k},\tilde{r})}$. Тогда рассматриваемая сумма примет вид
\begin{multline}
\sum_{x_3=0}^{\infty} \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) v^{x_3} = \sum_{x_3=L+1}^{\infty} \left[ Q_{3,i}(\Gamma^{(k,r\ominus_k 1)},x_3) + Q_{3,i}(\Gamma^{(0,r_2)},x_3) \right] v^{x_3} = \\=
\sum_{x_3=0}^{\infty} \left[ Q_{3,i}(\Gamma^{(k,r\ominus_k 1)},x_3) + Q_{3,i}(\Gamma^{(0,r_2)},x_3) \right] v^{x_3} -\sum_{x_3=0}^{L} \left[ Q_{3,i}(\Gamma^{(k,r\ominus_k 1)},x_3) + Q_{3,i}(\Gamma^{(0,r_2)},x_3) \right] v^{x_3} .
\end{multline}
И, наконец, если $\tilde{\gamma}\in C_{\tilde{k}}^{\mathrm{O}} \cup C_{\tilde{k}}^{\mathrm{N}}$, то $H_{-1}(\tilde{\gamma},x_3) = \{\Gamma^{(\tilde{k},\tilde{r}\ominus_{\tilde{k}} 1)}\}$ для всех $x_3\geqslant 0$. И рассматриваемая сумма в этом случае будет выглядеть следующим образом
\begin{multline}
\sum_{x_3=0}^{\infty} \sum_{\gamma \in H_{-1}(\tilde{\gamma},x_3)} Q_{3,i}(\gamma,x_3) v^{x_3} = \sum_{x_3=L+1}^{\infty} Q_{3,i}(\Gamma^{(k,r\ominus_k 1)},x_3) v^{x_3} =\\=
...
Начнем с двух небольших лемм.
\begin{lemma}
Марковская цепь $\MarkThree$ является ациклической.
\end{lemma}
\begin{proof}
Действительно, утверждение леммы сразу следует из определения циклической цепи (и периода)  \cite[ с. 536]{Shiryaev}, и того файта, что наибольший общий делитель $\text{НОД}(n_0,n_1,\ldots, n_d)$ равен $1$.
\end{proof}  

\begin{lemma}
Для величин $\alpha_i(k,r,u)$, $\Gamma^{(k,r)} \in \Gamma$, и некоторого фиксированного $0 < \varepsilon < 1$ верны следующие неравенства равномерно для всех $i \geqslant 0$ и $ 1-\varepsilon < u < 1  + \varepsilon$:
\begin{equation}
|\alpha_i(k,r,u)| \leqslant M(k,r),
\end{equation}
для некоторых конечных постоянных $M(k,r)$, $\Gamma^{(k,r)}\in \Gamma$, не зависящих от $u$ и $i$.
\end{lemma}
\begin{proof}
Начнем с оценки величины $\tilde{\alpha}_i(k,r,u)$, входящей в выражения для всех величин $\alpha_i(k,r,u)$. 
\begin{multline*}
|\tilde{\alpha}_i(k,r,v) |= | \sum_{x_3=0}^{\ell(k,r,3)}\sum_{\gamma \in H_{-1}(\Gamma^{(k,r)},x_3)} Q_{3,i}(\gamma,x_3) \sum_{a=0}^{\ell(k,r,3) - x_3} \varphi_3(a,T^{(k,r)}) - \\
- \sum_{x_3=0}^{\ell(k,r,3)}  \sum_{\gamma \in H_{-1}(\Gamma^{(k,r)},x_3)} Q_{3,i}(\gamma,x_3) v^{x_3-\ell(k,r,3)}  \sum_{w=0}^{\ell(k,r,3) -x_3}
\varphi_3(w,T^{(k,r)}) v^w| \leqslant \\ \leqslant (\ell(k,r,3) +1 ) + \sum_{x_3=0}^{\ell(k,r,3)} \sum_{\gamma \in H_{-1}(\Gamma^{(k,r)},x_3)} Q_{3,i}(\gamma,x_3) (1-\varepsilon)^{x_3-\ell(k,r,3)} \exp{(\lambda_3 T^{(k,r)} (f_3(1+\varepsilon) - 1))} \leqslant \\ \leqslant 
\ell(k,r,3) +1 + (1-\varepsilon)^{-\ell(k,r,3)} \exp{(\lambda_3 T^{(k,r)} (f_3(1+\varepsilon) - 1))} \sum_{x_3=0}^{\ell(k,r,3)} \sum_{\gamma \in H_{-1}(\Gamma^{(k,r)},x_3)} Q_{3,i}(\gamma,x_3)  \leqslant \\ \leqslant 
\ell(k,r,3) +1 + (1-\varepsilon)^{-\ell(k,r,3)} \exp{(\lambda_3 T^{(k,r)} (f_3(1+\varepsilon) - 1))} (\ell(k,r,3) +1)  = \tilde{M}(k,r)
\end{multline*}

Теперь рассмотрим оставшиеся слагаемые. Для $\Gamma^{(0,r)} \in \Gamma$
\begin{multline}
|\alpha_i(0,r,v)| = | \tilde{\alpha}_i(0,r,v) + q_{0,r}(v) \times \sum_{x_3=0}^{L} \left[ Q_{3,i}(\Gamma^{(k_1,r_1)},x_3) + Q_{3,i}(\Gamma^{(0,r\ominus_0 1)},x_3) \right] v^{x_3}  - \\ - q_{k,r}(v)\sum_{x_3=0}^{L} Q_{3,i}(\Gamma^{(0,r\ominus_0 1)},x_3) | \leqslant  | \tilde{\alpha}_i(0,r,v) | + |q_{0,r}(v) |\times \\ \times  (1+\varepsilon)^{L} \sum_{x_3=0}^{L}  \left[ Q_{3,i}(\Gamma^{(k_1,r_1)},x_3) + Q_{3,i}(\Gamma^{(0,r\ominus_0 1)},x_3) \right]   + |q_{k,r}(v)|\sum_{x_3=0}^{L} Q_{3,i}(\Gamma^{(0,r\ominus_0 1)},x_3) | 
\leqslant \\ \leqslant| \tilde{\alpha}_i(0,r,v) | + |v ^ {-\ell(k,r,3)} \exp{(\lambda_3 T^{(k,r)} (f_3(1+\varepsilon) - 1))}|\times (1+\varepsilon)^{L}  + \\ + |v ^ {-\ell(k,r,3)} \exp{(\lambda_3 T^{(k,r)} (f_3(1+\varepsilon) - 1))}| 
\leqslant 
\tilde{M}(k,r) + \\ +| (1-\varepsilon) ^ {-\ell(k,r,3)} \exp{(\lambda_3 T^{(k,r)} (f_3(1+\varepsilon) - 1))}|\times (1+\varepsilon)^{L}  + \\ + | (1-\varepsilon) ^ {-\ell(k,r,3)} \exp{(\lambda_3 T^{(k,r)} (f_3(1+\varepsilon) - 1))}|  = M(0,r)
\end{multline}

Для $\Gamma^{(k, r)} \in C_{k}^{\mathrm{I}}$
\begin{multline*}
|\alpha_i(k,r,v) |=|\tilde{\alpha}_i(k,r,v) - q_{k,r}(v)\sum_{x_3=0}^{L} \left[ Q_{3,i}(\Gamma^{(k,r\ominus_k 1)},x_3) + Q_{3,i}(\Gamma^{(0,r_2)},x_3) \right] v^{x_3}| \leqslant \\ \leqslant
|\tilde{\alpha}_i(k,r,v)|  + |q_{k,r}(v)| (1+\varepsilon)^{L} \sum_{x_3=0}^{L} \left[ Q_{3,i}(\Gamma^{(k,r\ominus_k 1)},x_3) + Q_{3,i}(\Gamma^{(0,r_2)},x_3) \right] \leqslant \\ \leqslant \tilde{M}(k,r)  + (1-\varepsilon) ^ {-\ell(k,r,3)} \exp{(\lambda_3 T^{(k,r)} (f_3(1+\varepsilon) - 1))} (1+\varepsilon)^{L} = M(k,r)
\end{multline*}

Для $\Gamma^{(k, r)} \in C_{k}^{\mathrm{O}} \cup C_{k}^{\mathrm{N}}$
\begin{multline*}
|\alpha_i(k,r,v)| = |\tilde{\alpha}_i(k,r,v) - q_{k,r}(v) \sum_{x_3=0}^{L} Q_{3,i}(\Gamma^{(k,r\ominus_k 1)},x_3) v^{x_3}| \leqslant \\ \leqslant 
 |\tilde{\alpha}_i(k,r,v)| + | q_{k,r}(v)|  (1+\varepsilon)^{L}  \sum_{x_3=0}^{L} Q_{3,i}(\Gamma^{(k,r\ominus_k 1)},x_3)| \leqslant \\ \leqslant
  \tilde{M}(k,r)+  (1-\varepsilon) ^ {-\ell(k,r,3)} \exp{(\lambda_3 T^{(k,r)} (f_3(1+\varepsilon) - 1))} (1+\varepsilon)^{L} = M(k,r).
\end{multline*}
...
\begin{theorem}
Для того, чтобы марковская цепь $\MarkThree$ имела стационарное распределение $Q(\gamma,x)$, $(\gamma,x)\in \Gamma \times Z_+$ Достаточно выполненеия неравенства 
$$\sum_{k,r \colon \Gamma^{(k,r)} \in \Gamma} \ell_{k,r,3} > \lambda_3 f_3'(1) \sum_{k,r \colon \Gamma^{(k,r)} \in \Gamma} T^{(k,r)}$$
\end{theorem}
\begin{proof}
Предположим обратное, а именно, что при $\sum_{k,r} \ell_{k,r,3} > \lambda_3 \sum_{k,r} T^{(k,r)}$ ациклическая марковская цепь $\MarkThree$ не имеет стационарного распределения. 
Тогда для любого состояния $(\gamma,x)\in \Gamma \times Z_+$ и независимо от начального распределения $\Pr(\Gamma_{0}=\Gamma^{(k,r)}, \varkappa_{3,0}=x)$,
$(\Gamma^{(k,r)},x)\in \Gamma \times Z_+$, 
имеют место предельные равенства 
\begin{equation}
\lim_{i \to \infty} \Pr(\Gamma_{i}=\Gamma^{(k,r)}, \varkappa_{3,i}=x) =0, \quad  (\Gamma^{(k,r)},x)\in \Gamma \times Z_+.
\label{zero:limit:equations}
\end{equation} 
Для доказательства этого факта достаточно рассмотреть все возможные случаи (см. рассуждения \cite[гл. $3$ пар.~3-4]{Shiryaev}):
\begin{enumerate}
...
Выберем это начальное распределение так, что при некотором $u_0 >1$  будет выполнено неравенство $\mathfrak{M}^{(0)}(k,r,u_0) <\infty$ для всех $\Gamma^{(k,r)}\in \Gamma$. Это ограничение, в силу теоремы \eqref{theorem:gen:rek}, обеспечивает при любом конечном $i\geqslant 0$ существование функций 
\begin{equation}
\mathfrak{M}^{(i)}(k,r,u),\quad \frac{d}{du} \left[\mathfrak{M}^{(i)}(k,r,u)\right]
\end{equation}
...
Другое рассуждение, однако, приводит к противоположному результату. Действительно, при $\sum_{k,r} \ell_{k,r,3} > \lambda_3 f_3'(1) \sum_{k,r} T^{(k,r)}$ имеем 
\begin{multline*}
 (\prod_{k,r}q_{k,r}(u)) ' |_{u=1} = 
 (\prod_{k,r}u^{-\ell(k,r,3)}\sum_{w=0}^{\infty} \varphi_3(w,T^{(k,r)})u^w )' |_{u=1} = \\ =
  (\prod_{k,r}u^{-\ell(k,r,3)}\exp(\lambda_3 T^{(k,r)} (f_3(u)-1)))' |_{u=1} = \\ =
    (u^{-\sum_{k,r}\ell(k,r,3)}\exp(\lambda_3 (f_3(u)-1))\sum_{k,r} T^{(k,r)} )' |_{u=1} = \\ =
\lambda_3 f_3'(1) \sum_{k,r} T^{(k,r)} -\sum_{k,r} \ell_{k,r,3}  < 0.
\end{multline*}
При $\mathfrak{M}_+^{(0)}(k,r,u) =\mathfrak{M}^{(0)}(k,r,u)$ в некоторй окрестности точки  $u = 1$  последовательности $\left\{\mathfrak{M}_+^{(i)}(k,r,u)\colon i \geqslant 0\right\}$, $\Gamma^{(k,r)} \in \Gamma$, рекуррентного отображения 
\begin{enumerate}
\item для $ \Gamma^{(0,r)} \in \Gamma$ $$\mathfrak{M}_+^{(i+1)}(0,r,u) = q_{0,r} (u)\times \mathfrak{M}_+^{(i)}(0,r \ominus_0 1,u) + M(0,r);$$
\item для $\Gamma^{(k, r)} \in C_{k}^{\mathrm{I}}$ и $h_3(r_2)=\Gamma^{(k,r)}$
$$\mathfrak{M}_+^{(i+1)}(k,r,u) = q_{k,r} (u)\times \left[ \mathfrak{M}_+^{(i)}(k,r \ominus_{k} 1,u) +  \mathfrak{M}_+^{(i)}(0,r_2,u) \right] + M(k,r);$$
\item для $\Gamma^{(k, r)} \in C_{k}^{\mathrm{O}} \cup C_{k}^{\mathrm{N}}$ 
$$\mathfrak{M}_+^{(i+1)}(k,r,u) = q_{k,r} (u)\times  \mathfrak{M}_+^{(i)}(k,r \ominus_{k} 1,u) + M(k,r);$$
\end{enumerate}
будут мажорантными соответственно для последовательностей $\left\{\mathfrak{M}^{(i)}(k,r,u)\colon i \geqslant 0\right\}$, $\Gamma^{(k,r)} \in \Gamma$, рекуррентного отображения из теоремы~\ref{theorem:gen:rek}.

На множестве пар $(k,r)$ таких, что $\Gamma^{(k,r)} \in \Gamma$, будем предполагать заданным лексикографический порядок: $(k_1,r_1) < (k_2, r_2)$ тогда и только тогда, когда $k_1 < k_2$ или $k_1 = k_2$ и $r_1 < r_2$. Тогда модуль определителя мажорантного отображения (в соответствии с введеным порядком компонент) лекго подсчитывается и будет равен
$|\prod_{k,r}q_{k,r}(u)|$. В точке $u=1$ модуль определителя $|\prod_{k,r}q_{k,r}(u)|$ равен $1$, а его производная равна $sgn(\prod_{k,r}q_{k,r}(1))\times (\lambda_3 f_3'(1) \sum_{k,r} T^{(k,r)} -\sum_{k,r} \ell_{k,r,3})$. Следовательно в некоторой правой окресности $u \in [1, 1 + \varepsilon_0)$ точки $u=1$ модуль определителя $|\prod_{k,r}q_{k,r}(u)|$ будет меньше $1$ и, значит, мажорантная последовательность сходится. Этот факт, в свою очередь, влечет сходимость исходной последовательности $\left\{\mathfrak{M}^{(i)}(k,r,u)\colon i \geqslant 0\right\}$, $\Gamma^{(k,r)} \in \Gamma$, для $u \in [1, 1 + \varepsilon)$.

Последовательности $\left\{\mathfrak{M}_+^{(i)}(k,r,u_1)\colon i \geqslant 0\right\}$, $\Gamma^{(k,r)} \in \Gamma$, сходятся при $u_1 \in (1,1+\varepsilon)$ и, следовательно, их сумма 
$\sum_{k,r} \mathfrak{M}^{(i)}(k,r,u)$ при любом $i\geqslant 0$ является аналитической, ограниченной  функцией.
И поскольку
\begin{equation}
\sum_{k,r} \mathfrak{M}^{(i)}(k,r,u) = \sum_{k,r} \sum_{w=0}^{\infty} Q_{3,i}(\Gamma^{(k,r)},w) u^w = 
 \sum_{w=0}^{\infty} \Pr(\varkappa_{3,i}=w) u^w,
\end{equation}
теперь без труда получается, что числовая последовательность 
$$
\left\{\sum_{k,r} \frac{d}{du}\left(\mathfrak{M}^{(i)}(k,r,u)\right)_{u=1}= E[\varkappa_{3,i}]; i\geqslant 0\right\} 
$$
в силу интегральной формулы Коши равномерно по $i$ ограничена некоторой постоянной величиной. Поэтому принятое в самом начале доказательства этой теоремы предположение не будет справедливым. Доказательство этим завершается.
\end{proof}
