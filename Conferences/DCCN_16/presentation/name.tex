...
\end{block}
% $\{(\Gamma_i(\omega), \varkappa_{3, i}(\omega)); i \geqslant 0\}$
\end{frame}
...
\framebreak
\begin{block}{\bf Теорема 4.}
Для того, чтобы марковская цепь $$\{(\Gamma_i(\omega), \varkappa_{3, i}(\omega)); i =0, 1, \ldots\}$$ имела стационарное распределение $Q_3(\gamma,x)$, $(\gamma,x)\in \Gamma \times {\mathbb Z}_+$, необходимо выполнение следующего неравенства
$$
\min_{k=\overline{1,d}} { \frac{\lambda_3 f_3'(1) \sum\limits_{r = 1}^{n_k} T^{(k,r)}}  {\sum\limits_{r = 1}^{n_{k}}\ell(k,r,3)}} <1.
$$
\label{theorem:nec}
\end{block}
\end{frame}
